...
public partial class pgeHome : System.Web.UI.Page
{
    ...
    protected void btndatabasebackup_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = new DataTable();
        // Backup destination
        string backupDestination = "D:\\SQLBackUpFolderHingalajCommodity";
        // check if backup folder exist, otherwise create it.
        if (!System.IO.Directory.Exists(backupDestination))
        {
            System.IO.Directory.CreateDirectory("D:\\SQLBackUpFolderHingalajCommodity");
        }
        try
        {
            sqlconn.Open();
            sqlcmd = new SqlCommand("backup database HingalajCommodity to disk='" + backupDestination + "\\" + "HingalajCommodity" + DateTime.Now.ToString("ddMMyyyy_HHmmss_") + user + ".Bak'", sqlconn);
